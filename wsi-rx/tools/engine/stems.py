...
# Windows defaults to cp1252 on redirected stdout, which explodes on the
# unicode arrows we emit in the summary lines (learned the hard way when
# td-audio002 completed separation fine but the success print crashed
# because the log file was opened with cp1252). Force utf-8 with a replace
# fallback so we never fail purely on a log write.
